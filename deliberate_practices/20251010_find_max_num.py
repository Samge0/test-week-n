...
# 模型路径
MODEL_PATH = os.path.join(CURRENT_DIR, 'models_output/model_20251010.pth')
...
def train():
    ...
    batch_size = int(train_config.get('batch_size', 50))
    barch_total = int(train_config.get('barch_total', 2000))
    current_date = train_config.get('current_date', '')
    lr = float(train_config.get('lr', 0.001))
    batch_max = math.ceil(barch_total/batch_size)
    
    # 训练数据+模型+优化器
    train_x, train_y = generate_datas(input_size, barch_total)
    ...
    swanlab.init(
        ...
        optimizer=optim,
        config=train_config,
        tags=["find_max_num", current_date, "test"],
    ...
    for epoch in range(epochs):
        ...
        indeices = torch.randperm(barch_total)
        train_x = train_x[indeices]
        train_y = train_y[indeices]
        
        loss_logs = []
        
        for i in range(batch_max):
            start_i = i * batch_size
            end_i = min(start_i + batch_size, barch_total)
        ...
        print(f"epoch: [{epoch+1}/{epochs}], acc: {acc:.4f}, loss: {mean_loss:.4f}")
