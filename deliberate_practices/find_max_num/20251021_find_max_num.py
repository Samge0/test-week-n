...
# 模型路径
MODEL_PATH = os.path.join(CURRENT_DIR, 'models_output/model_20251021.pth')
...
def generate_datas(size, total):
    x = np.random.random((total, size))
    y = np.argmax(x, axis=1)
    return torch.FloatTensor(x), torch.LongTensor(y)

def evaluate(model, size, total=500):
    model.eval()
    x, y = generate_datas(size, total)
    with torch.no_grad():
        y_perd = model(x)
