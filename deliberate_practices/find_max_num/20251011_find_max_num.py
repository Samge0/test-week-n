...
# 模型路径
MODEL_PATH = os.path.join(CURRENT_DIR, 'models_output/model_20251011.pth')
...
# 训练
def train():
    input_size = int(train_config["input_size"])
    epochs = int(train_config["epochs"])
    batch_size = int(train_config["batch_size"])
    batch_total = int(train_config["batch_total"])
    ...
    for epoch in range(epochs):
        ...
        print(f"epoch: [{epoch+1}/{epochs}], acc: {acc:.4f}, loss: {mean_loss}")
