...
# 模型路径
MODEL_PATH = os.path.join(CURRENT_DIR, 'models_output/model_20251027.pth')
...
def evaluate(model, input_size, total=500):
    ...
    with torch.no_grad():
        y_perd = model(x)
        return y_perd.argmax(dim=1).eq(y).sum().item() / total
...
def train():
    ...
    for epoch in range(epochs):
        ...
        mean_log = np.mean(loss_logs)
        train_logs.append([acc, mean_log])
        swanlab.log({"acc": acc, "loss": mean_log})
        print(f"epoch: [{epoch+1}/{epochs}], acc: {acc}, loss: {mean_log}")
